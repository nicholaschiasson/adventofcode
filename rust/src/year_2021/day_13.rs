    ...
    fn fold(&self, axis: &Axis) -> Self {
        Self {
            dots: self
                ...
                .iter()
                .map(|&d| match (*axis, d) {
                    (Axis::Horizontal(x_f), Point(x, y)) => {
                        Point(if x > x_f { x - (2 * (x - x_f)) } else { x }, y)
                    }
                    (Axis::Vertical(y_f), Point(x, y)) => {
                        Point(x, if y > y_f { y - (2 * (y - y_f)) } else { y })
                    }
                })
                .collect(),
        }
...
impl Display for Paper {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for y in 0..=max_y {
            for x in 0..=max_x {
                s.push(if self.dots.contains(&Point(x, y)) {
                    '#'
                } else {
                    '.'
                });
...
impl FromStr for Paper {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(Self {
            dots: HashSet::<Point>::from_iter(
                s.lines()
                    .map(|l| l.parse::<Point>().expect("failed to parse dot")),
            ),
        })
    }
}
