...
pub fn part_02(input: &String) -> u64 {
	let mut valid = input
		...
		.filter_map(|l| {
			...
			Some(s.iter().rev().fold(0, |p, &c| p * 5 + c.incomplete_points()))
		})
		.collect::<Vec<_>>();
	valid.sort();
	valid[valid.len() / 2]
...
mod tests {
	...
	#[test]
	fn part_01() {
		assert_eq!(super::part_01(&read_resource(relative_input_path(&format!("{}::practice_01", INPUT_PATH)))), 26397);
		assert_eq!(super::part_01(&read_resource(relative_input_path(&format!("{}::final", INPUT_PATH)))), 344193);
	}

	#[test]
	fn part_02() {
		assert_eq!(super::part_02(&read_resource(relative_input_path(&format!("{}::practice_01", INPUT_PATH)))), 288957);
		assert_eq!(super::part_02(&read_resource(relative_input_path(&format!("{}::final", INPUT_PATH)))), 3241238967);
	}
}
