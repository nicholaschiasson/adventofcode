		...
		.map(|(i, row)| {
			row
				...
				.map(|(j, s)| match s {
					...
					Space::EmptySeat => {
						if count_adjacent_occupancies(grid, Position(i as isize, j as isize), scan) == 0 {
							Space::OccupiedSeat
						} else {
							Space::EmptySeat
						}
...
mod tests {
	...
	fn part_01() {
		...
		assert_eq!(super::part_01(&read_resource(relative_input_path(&format!("{}::final", INPUT_PATH)))), 2470);
	}

	#[test]
	fn part_02() {
		assert_eq!(super::part_02(&read_resource(relative_input_path(&format!("{}::practice_01", INPUT_PATH)))), 26);
		assert_eq!(super::part_02(&read_resource(relative_input_path(&format!("{}::final", INPUT_PATH)))), 2259);
	}
}
