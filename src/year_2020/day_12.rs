...
impl ShipState {
	fn move_forward(self, distance: i64) -> Self {
		...
		Self(Point(x + (wx * distance), y + (wy * distance)), Point(wx, wy))
	}
...
impl FromStr for Action {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match &s[0..1] {
			"F" => Ok(Action::Forward(s[1..].parse().expect("Failed to parse forward action"))),
			"N" => Ok(Action::Move(
				Direction::North,
			...
			"L" => Ok(Action::Rotate(Rotation::Left(
				s[1..].parse::<i64>().expect("Failed to parse forward action") / 90,
			))),
...
pub fn part_01(input: &String) -> u64 {
	let Vector(_, Point(x, y)) = input
		...
		.fold(Vector(Direction::East, Point(0, 0)), |v, a| v.perform_action(a));
	(x.abs() + y.abs()) as u64
}
...
mod tests {
	...
	#[test]
	fn part_01() {
		assert_eq!(super::part_01(&read_resource(relative_input_path(&format!("{}::practice_01", INPUT_PATH)))), 25);
		assert_eq!(super::part_01(&read_resource(relative_input_path(&format!("{}::final", INPUT_PATH)))), 796);
	}

	#[test]
	fn part_02() {
		assert_eq!(super::part_02(&read_resource(relative_input_path(&format!("{}::practice_01", INPUT_PATH)))), 286);
		assert_eq!(super::part_02(&read_resource(relative_input_path(&format!("{}::final", INPUT_PATH)))), 39446);
	}
}
