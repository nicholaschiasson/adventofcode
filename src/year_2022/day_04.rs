...
impl FromStr for SectionAssignmentPair {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		Ok(SectionAssignmentPair(
			a1.parse::<u8>().expect("section bound a1 as number")..a2.parse::<u8>().expect("section bound a2 as number"),
			b1.parse::<u8>().expect("section bound b1 as number")..b2.parse::<u8>().expect("section bound b2 as number"),
		))
	}
}
...
pub fn part_01(input: &String) -> u64 {
	input
		.lines()
		.map(|l| l.parse::<SectionAssignmentPair>().expect("parse section assignment pair"))
