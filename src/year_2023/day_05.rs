                ...
                .flat_map(|r| {
                    m.lines()
                        ...
                        .find_map(|m| match (m[1]..m[1] + m[2], m[0]..m[0] + m[2], m[2]) {
                            ...
                            (src, dst, len) if src.contains(&r.start) => Some(vec![
                                dst.start + r.start - src.start..dst.start + len,
                                src.end..r.end,
                            ]),
                            (src, _, _) if src.contains(&r.end) => {
                                Some(vec![r.start..src.start, m[0]..m[0] + r.end - m[1]])
                            }
