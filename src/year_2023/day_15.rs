...
fn holiday_ascii_string_helper<S: AsRef<str>>(s: &S) -> u8 {
	s.as_ref().bytes().fold(0_u16, |h, b| ((h + b as u16) * 17) % 256) as u8
}
...
impl<K: AsRef<str> + Clone + PartialEq, V: Copy> Iterator for HolidayHashMapIntoIterator<K, V> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		let mut out = None;
		while self.index < self.holiday_hash_map.keys.len() && self.hash_index >= self.holiday_hash_map.keys[self.index].len() {
			self.index += 1;
			self.hash_index = 0;
		}
