		...
		.map(|(times, distances)| {
			times
				...
				.map(|(time, distance)| {
					...
					(time
						- 2.0 * (((time - (time.powf(2.0) - (4.0 * (((distance / 2.0).floor() * 2.0) + 1.0))).sqrt()) / 2.0).floor() + 1.0)
						+ 1.0) as u64
				})
				.product()
		})
	...
	input
		...
		.map(|(time, distance)| {
			(
				...
				distance.chars().filter(|c| !c.is_whitespace()).collect::<String>(),
			)
		})
		.map(|(time, distance)| {
			(
		...
		.map(|(time, distance)| {
			(time - 2.0 * (((time - (time.powf(2.0) - (4.0 * (((distance / 2.0).floor() * 2.0) + 1.0))).sqrt()) / 2.0).floor() + 1.0)
				+ 1.0) as u64
		})
		.unwrap()
}
