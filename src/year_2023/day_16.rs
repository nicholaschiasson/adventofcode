...
impl Tile {
	fn energize(&mut self, DumbVec((x, y), dir): DumbVec) -> Option<Vec<DumbVec>> {
		...
		match (self.space, dir) {
			...
			| (Space::Splitter(Axis::Vertical), Direction::Up | Direction::Down) => Some(vec![DumbVec(dir.incr_pos(x, y), dir)]),
			...
			(Space::Splitter(Axis::Vertical), Direction::Left | Direction::Right) => {
				Some(vec![DumbVec((x, y - 1), Direction::Up), DumbVec((x, y + 1), Direction::Down)])
			},
			(Space::Mirror(Angle::Forward), Direction::Up) => Some(vec![DumbVec((x + 1, y), Direction::Right)]),
			(Space::Mirror(Angle::Forward), Direction::Down) => Some(vec![DumbVec((x - 1, y), Direction::Left)]),
			(Space::Mirror(Angle::Forward), Direction::Left) => Some(vec![DumbVec((x, y + 1), Direction::Down)]),
			(Space::Mirror(Angle::Forward), Direction::Right) => Some(vec![DumbVec((x, y - 1), Direction::Up)]),
			(Space::Mirror(Angle::Backward), Direction::Up) => Some(vec![DumbVec((x - 1, y), Direction::Left)]),
			(Space::Mirror(Angle::Backward), Direction::Down) => Some(vec![DumbVec((x + 1, y), Direction::Right)]),
			(Space::Mirror(Angle::Backward), Direction::Left) => Some(vec![DumbVec((x, y - 1), Direction::Up)]),
			(Space::Mirror(Angle::Backward), Direction::Right) => Some(vec![DumbVec((x, y + 1), Direction::Down)]),
		}
	}
...
impl Grid {
	fn energize(&mut self, start: DumbVec) -> u64 {
		...
		while let Some(DumbVec((x, y), dir)) = queue.pop_front() {
			if !(0..self.tiles.len() as isize).contains(&y) || !(0..self.tiles[0].len() as isize).contains(&x) {
				continue;
			}
...
impl FromStr for Grid {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		Ok(Self {
			tiles: s.lines().map(|line| line.chars().map(Tile::from).collect()).collect(),
		})
	}
}
...
pub fn part_02(input: &str) -> u64 {
	...
	for i in 0..height {
		...
		max = max.max(grid.clone().energize(DumbVec((width - 1, i), Direction::Left)));
	}
	for i in 0..width {
		max = max.max(grid.clone().energize(DumbVec((i, 0), Direction::Down)));
		max = max.max(grid.clone().energize(DumbVec((i, height - 1), Direction::Up)));
	}
	max
}
